...
describe('shuffleBottles', () => {
  it('reorders tubes, the hidden grid, and the funnels by the same permutation', () => {
    ...
    const out = shuffleBottles(state, hidden, funnels, rng);

    // The multiset of (tube, hidden-row, funnel) triples is preserved — overlays still match tubes.
    const triples = out.state.bottles.map((b, i) => JSON.stringify([b, out.hidden[i], out.funnels[i]]));
    const original = state.bottles.map((b, i) => JSON.stringify([b, hidden[i], funnels[i]]));
    expect(triples.slice().sort()).toEqual(original.slice().sort());
  ...
  it('leaves solvability intact (it is a pure presentation transform)', () => {
    for (let seed = 0; seed < 25; seed++) {
      ...
      const out = shuffleBottles(level.state, empty, noFunnels(level.state), mulberry32(seed * 31 + 1));
  ...
  it('actually permutes (not always the identity) across rolls', () => {
    ...
    for (let seed = 0; seed < 20; seed++) {
      const out = shuffleBottles(state, empty, noFunnels(state), mulberry32(seed));
