 ...
 * the CLI shell. Usage:
 *
 *   npm run check                 # full gate (verify/e2e auto-skip when their inputs are absent)
 *   npm run check -- bake-out     # point the baked-level self-check at an artifacts dir
 *   npm run check -- --skip=e2e   # drop steps by id (e.g. to split CI into parallel jobs)
 */
import { resolveContext, runGate } from './gate';

process.exit(runGate(resolveContext(process.argv.slice(2))));
